<x-app-layout>
    ...
    <div class="space-y-4">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            @if ($documentos->isEmpty())
                <x-empty-state title="Nenhum documento exigido" description="Não há documentos configurados para esta função." />
            @else
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-1.5">
                                            @if ($doc->arquivo_caminho)
                                                <x-config-action-btn
                                                    :href="route('admin.docs-func.download', $doc)"
                                                    target="_blank"
                                                    variant="ghost"
                                                >
                                                ...
                                                <x-config-action-btn
                                                    ...
                                                    variant="primary"
                                                    x-data=""
                                                    x-on:click="$dispatch('open-modal', 'validar-func-{{ $doc->uuid }}')"
                                                >
                                                    Validar
                                                </x-config-action-btn>
                                            @else
    ...
    @foreach ($documentos as $doc)
        @if ($doc->arquivo_caminho)
            <x-modal name="validar-func-{{ $doc->uuid }}" focusable>
                <form method="POST" action="{{ route('admin.docs-func.validar', $doc) }}" class="p-6 space-y-5">
                    @csrf
                    <div>
                    ...
                    <div class="flex items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3">
                        ...
                        <a href="{{ route('admin.docs-func.download', $doc) }}" target="_blank" class="text-sm font-medium text-marino hover:underline">
