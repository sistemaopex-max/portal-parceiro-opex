<x-app-layout>
    <x-slot name="header">
        <x-page-heading
            title="Documentos — Funcionários"
            subtitle="Gerencie os documentos exigidos de cada funcionário"
        />
    </x-slot>
        ...
        <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-card">
            @if ($funcionarios->isEmpty())
                <x-empty-state
                    ...
                    description="Cadastre funcionários em Cadastros › Funcionários para gerenciar seus documentos."
                >
                    <x-slot name="actions">
                        <x-config-action-btn :href="route('parceiro.funcionarios.create')" variant="primary">
                    ...
                    <table class="min-w-full divide-y divide-zinc-100 text-sm">
                        <thead>
                            <tr class="bg-zinc-50/80">
                                <th class="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-400">Funcionário</th>
                        ...
                        <tbody class="divide-y divide-zinc-100">
                            @foreach ($funcionarios as $f)
                                ...
                                <tr class="transition hover:bg-zinc-50">
                                    ...
                                    <td class="px-6 py-4 text-zinc-500">{{ $f->funcao?->nome ?? '—' }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <x-config-action-btn :href="route('parceiro.funcionarios.docs.index', $f)" variant="primary">
